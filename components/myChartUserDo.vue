<template>
    <div class="mt-2 pt-0 custom-font">
        <canvas height="100px" id="myChartUserDo"></canvas>
    </div>
</template>
...
export default {
    ...
    methods: {
        createChart() {
            const ctx = document.getElementById('myChartUserDo').getContext('2d');
            if (this.myChartUserDo) {
                this.myChartUserDo.destroy();
            }
            // Sort userTarget array by Time_Care in descending order
            const sortedUserTarget = this.userTarget.slice(0, this.userTarget.length - 1).sort((a, b) => b.Time_Do - a.Time_Do);
            ...
            const valueCare = sortedUserTarget.map(item => item.Time_Do);
            ...
            const filteredValueDo = valueCare.filter(value => value > 0);
            const colorBK = [
                'rgb(51, 51, 51,0.8)',
                'rgb(255, 255, 0,0.8)',
                'rgba(255, 206, 86, 0.8)',
            ...
            const valueAll = Math.max(...filteredValueDo);
            const chartData = {
                labels: filteredLabels,
                datasets: [
                    {
                        label: 'Time Do Tplus > 20min',
                        data: filteredValueDo,
